...
import { getContact, getContactActivities, getContactMeetings, contacts } from "@/lib/mock-data";
import { stageColors, stageDotColors } from "@/lib/stage-colors";
import { PipelineStage, Activity } from "@/lib/types";
...
export default function ContactDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const activities = getContactActivities(id);
  const contactMeetings = getContactMeetings(id);
  const [stage, setStage] = useState<PipelineStage>(contact?.stage ?? "Lead");
  ...
  if (!contact) {
    return (
      <div className="p-8">
        ...
        <Link href="/contacts" className="text-sm text-primary hover:underline mt-2 inline-block">Back to contacts</Link>
      </div>
    );
  }
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-start gap-4">
                ...
                <Badge className={stageColors[contact.stage]} variant="secondary">{contact.stage}</Badge>
              </div>
              {contact.notes && (
