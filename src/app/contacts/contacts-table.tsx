...
import { FollowUpDropdown, FollowUpMenuItems } from "@/components/follow-up-dropdown";
        ...
        <div className="mb-4 p-3 rounded-lg border border-border bg-accent/50 flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            <FollowUpDropdown
              ...
              onSelect={(date) => { if (date) bulkSetFollowUp(date); }}
            />
            <button
              disabled={bulkAction}
            ...
            <tbody>
              {filtered.map((contact) => (
                <tr key={contact.id} className={`border-t border-border hover:bg-accent/50 transition-colors ${contact.archived ? "opacity-60" : ""}`}>
                  ...
                  <td className="px-4 py-3">
                    <FollowUpDropdown
                      value={contact.nextFollowUp || undefined}
                      onSelect={(date) => quickSetFollowUp(contact.id, date)}
                    />
                  </td>
